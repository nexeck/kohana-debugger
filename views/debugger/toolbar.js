...
if (typeof jQuery != 'undefined') {
	...
	$(document).ready(function () {
		...
		// bind ajax event
		$('#debug-ajax').bind("ajaxComplete", function (event, xhr, settings) {
			...
			$('#debug-ajax table').append(
							'<tr class="even">' +
											...
											'<td>' + xhr.statusText + ' (' + xhr.status + ')<\/td>' +
											'<td>' + settings.url + '<\/td>' +
											'<td><b><a href=="#" onclick="debugToolbar.toggle(\'ajax_response_' + $('#debug-ajax table tr').size() + '\'); return false;">Response Text</a></b><pre id="ajax_response_' +  $('#debug-ajax table tr').size() +'" style="display:none">' + xhr.responseText + '</pre></td>' +
											'<\/tr>'
			);
